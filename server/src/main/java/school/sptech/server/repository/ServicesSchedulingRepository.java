...
@Query("select new school.sptech.server.model.ServicesScheduling(s.id, s.workerId, s.customerId, s.nameService, s.serviceDescription, s.address, s.price, s.date, s.status) from ServicesScheduling s where s.customerId=?1 and (s.status=?2 or s.status=?3)")
