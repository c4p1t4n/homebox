...
public interface ServicesStatusRecentRepository  extends JpaRepository<ServicesStatusRecent, Integer> {
    ...
    @Query("select new school.sptech.server.model.ServicesStatusRecent(s.workerId, s.customerId, s.idScheduling, s.nameService, s.serviceDescription, s.address, s.price, s.date, s.status) from ServicesStatusRecent s where s.customerId=?1 and (s.status=?2 or s.status=?3)")
